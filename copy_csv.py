...
#'''
with open('test_data_weekday-3.csv', mode='w', newline='') as file:
    #selected_data_weekday.csv file contains expected data having following field
    writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)    
    writer.writerow(['starting_point_id','dropping_point_id','pickup_timestamp','dropoff_timestamp','pickup_time','pickup_time_min','duration','weekday'])
#'''
        ...
        with open('temp.csv','rt') as f:
            ...
            for row in data_file: 
                ...
                if (diff >= min_dur) and (diff <= max_dur):
                    ...
                    if diff < 100:
                        writer.writerow([row[7], row[8], row[1], row[2], c, total_min, diff, day])
